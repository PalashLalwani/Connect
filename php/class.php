...
class FUNCTIONS {
    ...
    public function complete_signup()
    {
        ...
        if(!($this->is_email_exist($email)))
        {
                ...
                $sql1 = "create table $fname(Text varchar(100))";
        ...
        {
            echo '<h1 style="color:red;">Try again. <a href="http://localhost/connect" class="btn btn-danger">Reload</a></h1>';
        }
    }
    ...
    public function nav()
    {
		...
		<nav class="navbar navbar-default">
			<div class="container-fluid">
				...
				<div class="navbar-header">
					...
					<a class="navbar-brand" href="home">CONNECT</a>
				</div>
		...
		<script>
			...
			function search_for(query) {
				if (query != "") {
					$.ajax({
						url: 'http://localhost/connect/php/search.php',
						method: 'GET',
						data: {
							'query': query
						},
        ...
        global $conn;
            ...
            if($query->rowCount()>0)
            {
                ...
                if($password == ($database_result->password))
                {
                    ...
                    ?>
			<script>
				window.location.href = 'http://localhost/connect/home';
			</script>
			<?php
                    exit;
                }
    ...
    public function show_glance($user_id)
    {
        ...
        ?>
				...
				<a style="text-decoration: none; color: #1f99f3;" href="http://localhost/connect/profile?id=<?php echo $user_id; ?>"><?php $this->get_full_name($user_id); ?></a>
				<?php
    }
